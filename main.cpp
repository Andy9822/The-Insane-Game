...
///Variaveis "globais" declaradas para poder usar na funcao de call back que reconhece teclas pressionadas da opengl
glm::vec4 camera_position_c =  glm::vec4(-1.5,0.7,0,1.0f);
...
void jorginho(float antigoX,float novoX,float antigoZ,float novoZ,float antigoY,bool *invadiuObjeto,std::vector<objeto> cubos);
int trataColisaoCubo(float novoX,float antigoY,float novoZ, int nearest,bool *invadiuObjeto,std::vector<objeto> cubos);
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(1200, 900, "INF01047", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    glfwSetWindowSize(window, 1200, 900); // Forçamos a chamada do callback acima, para definir g_ScreenRatio.
...
/// ---------------------------------------------------------------------------------------
    ...
    float antigoX = 0;
    float antigoZ = 0;
    float novoX = camera_position_c.x;
    float novoZ = camera_position_c.z;
    glm::vec4 nextPosition;
    glm::vec4 camera_view_vector;
    float deslocamento = 0.025;
...
/// ---------------------------------------------------------------------------------------
    ...
    {
        actualSecond = (int)(glfwGetTime() * 10);
        ...
        // Pedimos para astartJump GPU utilizar o programa de GPU criado acima (contendo
        ...
        ///Ate aqui a parte de atualizar matrizes, na funcao de call back tem o reconhecimento das telcas pressionadas
        ...
        for (int i = 0; i < int (cubos.size()) ; ++i)
        {
            ...
            model = Matrix_Translate(cubos[i].x, cubos[i].y, cubos[i].z)
                    * Matrix_Scale(cubos[i].dx, cubos[i].dy, cubos[i].dz); // PRIMEIRO escala

            if ( i == int (cubos.size()) - 1 )
            {
    ...
    // Fim do programa
    return 0;/// l vai ser diferente de -1 se a nova posicao x,z estar em cima de algum cubo
}
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    // Em coordenadas esféricas, o ângulo phi deve ficar entre,glm::vec4 w -pi/2 e +pi/2.
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    ///Teste de se apertou alguma das teclas WASD
    ...
    ///Teste de se deixou de apertar alguma das teclas WAS
...
///Testa se uma coordenada de 1 eixo esta abaixo da mesma de uma plataforma considerando seu delta de escalamento
bool entreLimites(float posCamera, float eixo,float delta, float erro)
{
    bool entre = false;
...
void processaWASD(float *novoX,float antigoX,float *novoZ,float antigoZ,float deslocamento,glm::vec4 u,glm::vec4 w)
{
    ...
    if (pressW)
    {
        (*novoX) = antigoX + deslocamento*-w.x;
        (*novoZ) = antigoZ + deslocamento*-w.z;
    }
    if (pressS)
    {
        (*novoX) = antigoX + deslocamento*w.x;
    ...
    if (pressA)
    {
        (*novoX) = antigoX + deslocamento*-u.x;
        (*novoZ) = antigoZ + deslocamento*-u.z;
    }
    if (pressD)
    {
        (*novoX) = antigoX + deslocamento*u.x;
...
///Retorna o indice da plataforma abaixo do personagem. -1 se empty
int trataColisaoCubo(float novoX,float antigoY,float novoZ, int nearest,bool *invadiuObjeto,std::vector<objeto> cubos)
{
    std::cout << "near " << nearest <<std::endl;
    std::cout << "old " << oldCubo <<std::endl;
    if(nearest == oldCubo)
    {
    ...
    else
    {
        if(oldCubo == -1)
        {
            ...
            {
                if (entreLimites(camera_position_c.y,cubos[nearest].y,cubos[nearest].dy,0.0f))
                {
        ...
        {
            std::cout << "invadi " << nearest <<std::endl;
            std::cout << "meu y " << camera_position_c.y - 0.2 <<std::endl;
            //std::cout <<entreLimites(camera_position_c.y,cubos[nearest].y,cubos[nearest].dy,0.0f) <<std::endl;
            //std::cout <<  (sup > camera_position_c.y - 0.2 && camera_position_c.y - 0.2 > inf) <<std::endl<<std::endl;
            ...
            std::cout << "invadiu = " << *invadiuObjeto<<std::endl<<std::endl<<std::endl;
        }
...
int cuboProximo(float novoX,float antigoY,float novoZ,std::vector<objeto> cubos)
{
    ...
    for(int i = 0; i< cubos.size() ; i++)
    {
        if(entreLimites(novoX,cubos[i].x,cubos[i].dx,0.0f) )
        {
            ...
            {
                aux = camera_position_c.y - (cubos[i].y + cubos[i].dy/2 + 0.2 );
                if(  aux < longe )
                {
                    indice = i;
                    longe = aux;
...
void processaMovimentos(bool WASD,float antigoX,float novoX,float antigoZ,float novoZ,float antigoY,std::vector<objeto> cubos)
{
    ...
    if(WASD || CAINDO || JUMPING)
    {
        ...
        //std::cout << "X atual: " << camera_position_c.x << std::endl ;


        if(DIED)
        {
            camera_position_c =   glm::vec4(-1.5,0.7,0,1.0f);
            CAINDO = false;
        }
